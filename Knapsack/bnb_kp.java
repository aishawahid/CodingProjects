...
public class bnb_kp extends KnapSack {
	...
	// store it in the priority queue
		struc_sol empty = new struc_sol();
		frac_bound(empty, 0);
		int current_best = empty.val;
		insert(empty);

		while ((QueueSize > 0) && current_best < pqueue[1].bound){
			struc_sol max = removeMax();
			struc_sol left = max.copy();
			struc_sol right = max.copy();
			left.solution_vec.set(left.fixed+1, true);
			left.fixed = left.fixed +1;
			right.solution_vec.set(right.fixed+1, false);
			right.fixed = right.fixed +1;

			frac_bound(left, left.fixed);
			if (left.val != -1){
					insert(left);
			}
			if (left.val > current_best)
			{
				current_best = left.val;
				copy_array(left.solution_vec, final_sol);
			}

			frac_bound(right, right.fixed);
			if (right.val != -1){
				insert(right);
			}
			if (right.val > current_best)
			{
				current_best = right.val;
				copy_array(right.solution_vec, final_sol);
			}

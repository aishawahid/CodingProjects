...
public class ZekielsPracticalTestQuest extends AbstractQuest {
	...
	private void practicalTestStep() {
		...
		/*
		 * npc.add(ConversationStates.ATTENDING, "send", new AndCondition( new
		 * QuestInStateCondition(QUEST_SLOT,"candles_done"), new NotCondition(new
		 * PlayerHasItemWithHimCondition("candle"))), ConversationStates.IDLE, null, new
		 * MultipleActions( new SetQuestAction(QUEST_SLOT, "first_step"), new
		 * TeleportAction("int_semos_wizards_tower_1", 15, 16, Direction.DOWN), new
		 * CandleCheckAction(new StendhalRPZone("int_semos_wizards_tower_1"))) );
		 */
	}
